      ...
      <AnimatePresence mode="wait">
        ...
        {state === "empty" && !uploadError && (
          ...
          >
            ...
            <p className="mb-6 max-w-[280px]" style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 400, fontStyle: "italic", fontSize: "14px", color: "var(--muted)", lineHeight: 1.5 }}>
            ...
            <div className="grid grid-cols-2 gap-3 w-full mb-6">
              {["Reviewed by doctor", "Personalized", "3-5 business days", "Private & secure"].map((f) => (
                <div key={f} className="rounded-lg p-3 text-center" style={{ backgroundColor: "var(--card2)", fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 900, fontSize: "11px", letterSpacing: "1px", color: "var(--text-soft)" }}>
                  {f.toUpperCase()}
                </div>
              ))}
            </div>
            <div className="flex flex-col w-full gap-3">
              <button
                ...
                style={{ backgroundColor: "var(--red)", fontFamily: "'Anton', sans-serif", fontWeight: 400, fontSize: "14px", letterSpacing: "1px" }}
              ...
              {fileValidationError && (
                <p style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 400, fontStyle: "italic", fontSize: "13px", color: "#C41101", textAlign: "center" }}>
                  {fileValidationError}
                </p>
              )}
              <button
                className="w-full py-3 rounded-lg transition-colors hover:opacity-80"
                style={{ backgroundColor: "transparent", border: "1.5px solid var(--border-subtle)", color: "var(--text)", fontFamily: "'Anton', sans-serif", fontWeight: 400, fontSize: "13px", letterSpacing: "1px" }}
        ...
        {state === "uploading" && (
          <motion.div key="uploading" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="app-card flex flex-col items-center text-center !p-10">
            ...
            <p className="mb-6" style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 400, fontStyle: "italic", fontSize: "13px", color: "var(--muted)" }}>
            ...
            <p className="mt-2" style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: "11px", color: "var(--muted)" }}>
              {Math.round(progress)}%
            </p>
          </motion.div>
        )}
        ...
        {state === "error" && uploadError && (
          <motion.div key="error" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="app-card !p-0 overflow-hidden">
            <div style={{ background: "#FEF2F2", border: "1px solid #FECACA", borderRadius: "14px", padding: "20px", textAlign: "center" }}>
              ...
              <div style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 400, fontStyle: "italic", fontSize: "13px", color: "#7F1D1D", lineHeight: 1.5, marginBottom: uploadError.hint ? "10px" : "16px" }}>
                {uploadError.message}
              </div>
              {uploadError.hint && (
                <div style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 400, fontStyle: "italic", fontSize: "12px", color: "#6E665C", background: "rgba(0,0,0,.04)", borderRadius: "8px", padding: "10px 12px", marginBottom: "16px", lineHeight: 1.4 }}>
                  {uploadError.hint}
                </div>
              )}
              <button
                onClick={() => { setUploadError(null); setState("empty"); fileInputRef.current?.click(); }}
                style={{ background: "#C41101", color: "#fff", border: "none", borderRadius: "10px", padding: "10px 24px", fontFamily: "'Anton', sans-serif", fontWeight: 400, fontSize: "14px", letterSpacing: "1px", cursor: "pointer" }}
          ...
          <motion.div key="success" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="app-card !p-6 flex flex-col gap-5">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 400, fontStyle: "italic", fontSize: "14px", color: "var(--text)", lineHeight: 1.5, marginTop: "2px" }}>{result.summary}</p>
              </div>
            </div>

            {result.metrics && result.metrics.length > 0 && (
              <div>
                <p className="mb-2" style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 900, fontSize: "10px", letterSpacing: "1.5px", color: "var(--muted)" }}>HASIL PEMERIKSAAN</p>
                  ...
                  {result.metrics.map((m, i) => {
                    ...
                    return (
                      <div key={i} className="rounded-lg p-3" style={{ backgroundColor: "var(--card2)" }}>
                        ...
                        <p style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: "14px", fontWeight: 400, color: "var(--text)" }}>{m.value}</p>
                        {m.note && <p style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 400, fontStyle: "italic", fontSize: "11px", color: "var(--muted)", marginTop: "2px" }}>{m.note}</p>}
                      </div>
                    );
                  })}
                </div>
            ...
            {result.doctor_notes && (
              <div className="rounded-lg p-4" style={{ backgroundColor: "var(--card2)", borderLeft: "3px solid var(--red)" }}>
                ...
                <p style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 400, fontStyle: "italic", fontSize: "13px", color: "var(--text-soft)", lineHeight: 1.5 }}>{result.doctor_notes}</p>
              </div>
            )}

            {result.recommendations && result.recommendations.length > 0 && (
              <div>
                <p className="mb-2" style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 900, fontSize: "10px", letterSpacing: "1.5px", color: "var(--muted)" }}>REKOMENDASI</p>
                <div className="flex flex-col gap-1.5">
                  {result.recommendations.map((r, i) => (
                    <div key={i} className="flex items-start gap-2">
                      <div className="w-1.5 h-1.5 rounded-full mt-1.5 shrink-0" style={{ backgroundColor: "var(--red)" }} />
                      <p style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 400, fontStyle: "italic", fontSize: "13px", color: "var(--text-soft)", lineHeight: 1.4 }}>{r}</p>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="flex items-center justify-between pt-3" style={{ borderTop: "1px solid var(--border-subtle)" }}>
              <p style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 400, fontStyle: "italic", fontSize: "11px", color: "var(--muted)" }}>Reviewed by AI · {result.reviewed_at}</p>
              <button
                onClick={() => fileInputRef.current?.click()}
                className="flex items-center gap-1 transition-opacity hover:opacity-70"
                style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 400, fontStyle: "italic", fontSize: "12px", color: "var(--muted)" }}
        ...
        {showConfirmation && (
          ...
          >
            ...
            >
              ...
              <h2 style={{ fontFamily: "'Anton', sans-serif", fontWeight: 400, fontSize: 22, letterSpacing: "1px", color: "#0A0908", marginBottom: 16 }}>
              ...
              {nameMismatch && detectedName && (
                <div style={{ background: "#FEF3C7", border: "1px solid #FCD34D", borderRadius: "12px", padding: "14px 16px", marginBottom: "16px", display: "flex", gap: "10px", alignItems: "flex-start" }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 400, fontStyle: "italic", fontSize: "13px", color: "#78350F", lineHeight: 1.4 }}>
              ...
              <div style={{ background: "#fff", borderRadius: 10, padding: "14px 16px", marginBottom: 20, display: "flex", flexDirection: "column", gap: 6 }}>
                {detectedName && (
                  <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                    ...
                    <span style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 400, fontSize: 13, color: "#0A0908" }}>{detectedName}</span>
                  </div>
                )}
                <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                  ...
                  <span style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 400, fontStyle: "italic", fontSize: 13, color: "#0A0908" }}>Medical Checkup / Lab Result</span>
                </div>
                {pendingResult?.grade && (
                  <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                    <span style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: 11, letterSpacing: "1.5px", color: "#6E665C", minWidth: 130 }}>GRADE KESEHATAN</span>
                    <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 13, fontWeight: 400, color: gradeColor[pendingResult.grade] }}>{pendingResult.grade}</span>
                  </div>
                )}
              </div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 12, marginBottom: 24 }}>
                {[
                  ...
                  <label key={i} style={{ display: "flex", alignItems: "flex-start", gap: 12, cursor: "pointer" }}>
                    ...
                    <span
                      onClick={() => setChecks((prev) => prev.map((v, idx) => (idx === i ? !v : v)))}
                      style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 400, fontStyle: "italic", fontSize: 13, color: "#0A0908", lineHeight: 1.5 }}
                    >
                      {label}
                    </span>
                  </label>
              ...
              <button
                ...
                style={{
                  ...
                  fontFamily: "'Anton', sans-serif", fontWeight: 400, fontSize: 14, letterSpacing: "1.5px",
              ...
              <button
                ...
                style={{ display: "block", width: "100%", marginTop: 12, background: "none", border: "none", cursor: "pointer", fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 400, fontStyle: "italic", fontSize: 13, color: "#9E9891", textAlign: "center" }}
              >
                Batal
              </button>
            </motion.div>
